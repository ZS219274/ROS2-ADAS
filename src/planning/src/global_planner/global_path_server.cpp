...
GlobalPathServer::GlobalPathServer()
    : Node("global_path_server_node")  // 全局路径服务器节点
{
  ...
  // 初始化发布器for rviz
  global_path_rviz_pub_ =
      this->create_publisher<Marker>("global_path_rviz", 10);

  // 初始化服务端
  global_path_server_ = this->create_service<GlobalPathService>(
      "global_path_service",
      std::bind(&GlobalPathServer::response_global_path_callback, this, _1,
                _2));
}
...
void GlobalPathServer::response_global_path_callback(
  ...
  if (request->pnc_map.midline.points.empty()) {
    RCLCPP_WARN(this->get_logger(),
                "pnc_map is empty, global path can not be created!");
    return;
  }

  const auto global_path =
      global_planner_creator_->search_global_path(request->pnc_map);
