...
namespace Planning
{
  ...
  private:
    ...
    void draw_arc_y_mirror(const double &angle, const double &plus_flag, const double &ratio = 1.0); // 关于y轴对称的画弧线

    // 新增交叉路口专用方法
    void draw_crossroads(Point &s_edge_point, Point &n_edge_point, Point &e_edge_point);     // 绘制十字型交叉路口
    void draw_t_junction(Point &s_edge_point, Point &n_edge_point, Point &e_edge_point, const double &plus_flag);     // 绘制T形路口 
  };
} // namespace Planning
