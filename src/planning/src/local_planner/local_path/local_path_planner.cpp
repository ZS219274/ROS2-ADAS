...
{
  RCLCPP_INFO(rclcpp::get_logger("local_path_planner.cpp"),
              "局部路径规划已创建");
...
// 生成局部路径
LocalPath LocalPathPlanner::create_local_path(
    const Referline& reference_line,
  ...
  for (int i = 0; i < local_path_config_->local_path().path_size_; i++) {
    // 规划的起点
    point_s += car->ds_dt();  // 路径规划的起点：下一帧参考线投影点的s
                              // 速度单位：m/帧，不是m/s
    if (point_s > reference_line.refer_line.back()
                      .rs)  // 如果投影点超过了参考线最前端，就跳出
    {
      break;
    }
    ...
    for (int j = 0; j < sl_points_size - 1; j++) {
      ...
      if (point_s >= start_s && point_s < end_s) {
        ...
        const Eigen::Vector<double, 6> a = PolynomialCurve::quintic_polynomial(
            start_s, start_l, start_dl_ds, start_ddl_ds, end_s, end_l,
            end_dl_ds, end_ddl_ds);
        point_tmp.l = a(0) + a(1) * point_s + a(2) * point_s_2 +
                      a(3) * point_s_3 + a(4) * point_s_4 + a(5) * point_s_5;
        point_tmp.dl_ds = a(1) + 2.0 * a(2) * point_s + 3.0 * a(3) * point_s_2 +
                          4.0 * a(4) * point_s_3 + 5.0 * a(5) * point_s_4;
        point_tmp.ddl_ds = 2.0 * a(2) + 6.0 * a(3) * point_s +
                           12.0 * a(4) * point_s_2 + 20.0 * a(5) * point_s_3;
      }
    }
  ...
  for (auto& point : local_path_.local_path) {
    ...
    const double rx =
        reference_line.refer_line[match_index].pose.pose.position.x;
    const double ry =
        reference_line.refer_line[match_index].pose.pose.position.y;
    ...
    Curve::frenet_to_cartesian(point.s, point.ds_dt, point.dds_dt, point.l,
                               point.dl_ds, point.ddl_ds, rs, rx, ry, rtheta,
                               rkappa, rdkappa, x, y, theta, speed, a, kappa);
  ...
  RCLCPP_INFO(rclcpp::get_logger("local_path_planner.cpp"),
              "局部路径投影点参数计算完成,size = %ld",
              local_path_.local_path.size());
...
void LocalPathPlanner::init_local_path() {
  local_path_.header.frame_id =
      local_path_config_->pnc_map().frame_;          // 设置坐标系
