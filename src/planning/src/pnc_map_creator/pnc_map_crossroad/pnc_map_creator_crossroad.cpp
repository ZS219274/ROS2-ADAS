...
PncMapCreatorCrossroad::PncMapCreatorCrossroad() {
  RCLCPP_INFO(rclcpp::get_logger("pnc_map_creator_crossroad.cpp"),
              "PncMapCreatorCrossroad created.");

  // 读取配置文件, 给参数赋值
  pnc_map_config_ = std::make_unique<ConfigReader>();
  pnc_map_config_->read_pnc_map_config();
  map_type_ = static_cast<int32_t>(PncMapType::kCROSSROAD) +
              1;  // 在原有基础上增加新的类型
...
PNCMap PncMapCreatorCrossroad::create_pnc_map() {
  ...
  RCLCPP_INFO(rclcpp::get_logger("pnc_map_creator_crossroad.cpp"),
              "pnc_map created,midline points size: %ld ------ ",
              pnc_map_.midline.points.size());
...
void PncMapCreatorCrossroad::draw_straight_x(const double& length,
                                             const double& plus_flag,
                                             const double& ratio)  // 画直道x
{
  double len_tmp = 0.0;
...
void PncMapCreatorCrossroad::draw_straight_y(const double& length,
                                             const double& plus_flag,
                                             const double& ratio)  // 画直道y
{
  double len_tmp = 0.0;
  while (len_tmp < length) {
    if (plus_flag == 1.0) {
...
void PncMapCreatorCrossroad::draw_arc_x(
    const double& angle, const double& plus_flag,
    const double&
        ratio)  // 画弧线， 逆时针为正方向，顺时针为负方向， angele为总角度
{
  double theta_tmp = 0.0;
  while (theta_tmp < angle) {
    // 计算 左中右三点坐标
    pl_.x = p_mid_.x - std::sin(theta_current_) *
                           pnc_map_config_->pnc_map().road_half_width_;
    pl_.y = p_mid_.y + std::cos(theta_current_) *
                           pnc_map_config_->pnc_map().road_half_width_;
    pr_.x = p_mid_.x + std::sin(theta_current_) *
                           pnc_map_config_->pnc_map().road_half_width_;
    pr_.y = p_mid_.y - std::cos(theta_current_) *
                           pnc_map_config_->pnc_map().road_half_width_;
    // 将三点存储
    pnc_map_.midline.points.emplace_back(p_mid_);
    pnc_map_.left_boundary.points.emplace_back(pl_);
...
void PncMapCreatorCrossroad::draw_arc_y(
    const double& angle, const double& plus_flag,
    const double& ratio)  // 沿y方向展开的画弧线
{
...
void PncMapCreatorCrossroad::draw_arc_y_mirror(
    const double& angle, const double& plus_flag,
    const double& ratio)  // 关于y轴对称的画弧线
{
  // 保存原始的theta_current_值
  double original_theta_current = theta_current_;
...
void PncMapCreatorCrossroad::draw_crossroads(
    Point& s_edge_point, Point& n_edge_point,
    Point& e_edge_point)  // 绘制T字型交叉路口
{
  // 可调整的长度参数（单位与地图一致）
...
void PncMapCreatorCrossroad::draw_t_junction(Point& s_edge_point,
                                             Point& n_edge_point,
                                             Point& e_edge_point,
                                             const double& plus_flag) {
  // 可调整的长度参数（单位与地图一致）
