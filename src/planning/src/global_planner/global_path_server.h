...
#include "global_planner_astar/global_planner_astar.h"
...
class GlobalPathServer : public rclcpp::Node {
 ...
 private:
  std::shared_ptr<Planning::GlobalPlannerBase>
      global_planner_creator_;                          // 全局规划创建器
  rclcpp::Publisher<Path>::SharedPtr global_path_pub_;  // 全局路径发布器
  rclcpp::Publisher<Marker>::SharedPtr
      global_path_rviz_pub_;  // 全局路径地图Marker发布器
  rclcpp::Service<GlobalPathService>::SharedPtr
      global_path_server_;  // 全局路径服务器
};
}  // namespace Planning
