...
void Curve::cartesian_to_frenet(
    const double& x, const double& y, const double& theta, const double& speed,
    const double& a,
    const double& kappa,  // 输入1：目标点在笛卡尔下的参数 x y theta speed a
    const double& rs, const double& rx, const double& ry, const double& rtheta,
    const double& rkappa,
    const double& rdkappa,  // 输入2：目标点在参考线的投影点在笛卡尔下的参数 rs
                            // rx ry rtheta rkappa rdkappa
    double& s, double& ds_dt, double& dds_dt, double& l, double& dl_ds,
    double& dl_dt, double& ddl_ds,
    double& ddl_dt)  // 输出：目标点在自然坐标系下的参数 s ds_dt dds_dt l dl_ds
                     // dl_dt ddl_ds ddl_dt
{
  // 计算s
  s = rs;
  ...
  const double delta_theta_prime =
      one_minus_kappa_l / cos_delta_theta * kappa - rkappa;
  ddl_ds = -kappa_l_prime * tan_delta_theta +
           one_minus_kappa_l / (cos_delta_theta * cos_delta_theta) *
               delta_theta_prime;
  ...
  // 计算d(ds)/dt
  dds_dt = (a * cos_delta_theta -
            (ds_dt * ds_dt) * (dl_ds * delta_theta_prime - kappa_l_prime)) /
           one_minus_kappa_l;
...
void Curve::frenet_to_cartesian(
    const double& s, const double& ds_dt, const double& dds_dt, const double& l,
    const double& dl_ds,
    const double& ddl_ds,  // 输入1：目标点在自然坐标系下的参数 s ds_dt dds_dt l
                           // dl_ds dl_dt ddl_ds ddl_dt
    const double& rs, const double& rx, const double& ry, const double& rtheta,
    const double& rkappa,
    const double& rdkappa,  // 输入2：目标点在参考线的投影点在笛卡尔下的参数 rs
                            // rx ry rtheta rkappa rdkappa
    double& x, double& y, double& theta, double& speed, double& a,
    double& kappa)  // 输出：目标点在笛卡尔下的参数 x y theta speed a
{
  // 判断 s 和 rs 是否足够近
  if (std::fabs(rs - s) > delta_s_min) {
    RCLCPP_ERROR(rclcpp::get_logger("curve.cpp"),
                 "s 和 rs 距离太远,无法转换! rs = %.2f, s = %.2f", rs, s);
    return;
  }
...
// 找匹配点的下标（利用上一帧)
int Curve::find_match_point(const Path& path, const int& last_match_point_index,
       ...
       i++)  // 计算车辆定位点到路径点的距离，找出最近的投影点
  {
    ...
    if (dis < min_dis) {
      if (abs(last_match_point_index - i) >
          100)  // 如果距离大于100，则跳过,这是特殊情况，如路径交叉
      {
        continue;
      }
      min_dis = dis;
...
// 找匹配点的下标(在参考线上查找匹配点)
int Curve::find_match_point(const Referline& refer_line,
                            const PoseStamped& target_point) {
  ...
  for (int i = 0; i < path_size;
       i++)  // 计算车辆定位点到路径点的距离，找出最近的投影点
  {
    double dis = std::hypot(refer_line.refer_line[i].pose.pose.position.x -
...
// 在局部路径上查找匹配点下标 116
int Curve::find_match_point(const LocalPath& path,
                            const PoseStamped& target_point) {
  ...
  {
    double dis = std::hypot(
        path.local_path[i].pose.pose.position.x - target_point.pose.position.x,
...
void Curve::find_projection_point(
    const Referline& referline,
    const PoseStamped& target_point,  // 输入：参考线和目标点
    double& rs, double& rx, double& ry, double& rtheta, double& rkappa,
    double& rdkappa)  // 输出：目标点在参考线的投影点在笛卡尔下的参数 rs rx ry
...
void Curve::find_projection_point(
    ...
    const PoseStamped& target_point,  // 输入：局部路径和目标点
    double& rs, double& rx, double& ry, double& rtheta, double& rkappa,
    double& rdkappa)  // 输出：目标点在局部路径的投影点在笛卡尔下的参数 rs rx ry
                      // rtheta rkappa rdkappa
{
  // 简化：用匹配点近似代替  前提：参考线足够密且足够平滑
...
void Curve::cal_projection_param(Referline& refer_line) {
  ...
  for (int i = 0; i < path_size; i++) {
    if (i == 0) {
      ...
      rs += std::hypot(refer_line.refer_line[i].pose.pose.position.y -
  ...
  for (int i = 0; i < path_size; i++) {
    if (i < path_size - 1) {
      refer_line.refer_line[i].rtheta =
          std::atan2(refer_line.refer_line[i + 1].pose.pose.position.y -
                         refer_line.refer_line[i].pose.pose.position.y,
                     refer_line.refer_line[i + 1].pose.pose.position.x -
                         refer_line.refer_line[i].pose.pose.position.x);
    } else {
      refer_line.refer_line[i].rtheta =
          std::atan2(refer_line.refer_line[i].pose.pose.position.y -
                         refer_line.refer_line[i - 1].pose.pose.position.y,
                     refer_line.refer_line[i].pose.pose.position.x -
                         refer_line.refer_line[i - 1].pose.pose.position.x);
    }
  ...
  for (int i = 0; i < path_size; i++) {
    if (i < path_size - 1) {
      const double dis =
          std::hypot(refer_line.refer_line[i + 1].pose.pose.position.y -
                         refer_line.refer_line[i].pose.pose.position.y,
                     refer_line.refer_line[i + 1].pose.pose.position.x -
                         refer_line.refer_line[i].pose.pose.position.x);
      if (dis <= kMathEpsilon) {
        ...
        refer_line.refer_line[i].rkappa = (refer_line.refer_line[i + 1].rtheta -
                                           refer_line.refer_line[i].rtheta) /
                                          dis;
      }
    } else {
      const double dis =
          std::hypot(refer_line.refer_line[i].pose.pose.position.y -
                         refer_line.refer_line[i - 1].pose.pose.position.y,
                     refer_line.refer_line[i].pose.pose.position.x -
                         refer_line.refer_line[i - 1].pose.pose.position.x);
      if (dis <= kMathEpsilon) {
        ...
        refer_line.refer_line[i].rkappa =
            (refer_line.refer_line[i].rtheta -
             refer_line.refer_line[i - 1].rtheta) /
            dis;
      }
    }
  }
  ...
  for (int i = 0; i < path_size; i++) {
    if (i < path_size - 1) {
      ...
      if (dis <= kMathEpsilon) {
        ...
        refer_line.refer_line[i].rkappa = (refer_line.refer_line[i + 1].rkappa -
                                           refer_line.refer_line[i].rkappa) /
                                          dis;
      }
    } else {
      ...
      if (dis <= kMathEpsilon) {
        ...
        refer_line.refer_line[i].rkappa =
            (refer_line.refer_line[i].rkappa -
             refer_line.refer_line[i - 1].rkappa) /
            dis;
      }
    }
  }
...
void Curve::cal_projection_param(LocalPath& local_path) {
  ...
  for (int i = 0; i < path_size; i++) {
    if (i == 0) {
      ...
      rs += std::hypot(local_path.local_path[i].pose.pose.position.y -
                           local_path.local_path[i - 1].pose.pose.position.y,
                       local_path.local_path[i].pose.pose.position.x -
                           local_path.local_path[i - 1].pose.pose.position.x);
    }
  ...
  for (int i = 0; i < path_size; i++) {
    if (i < path_size - 1) {
      const double dis =
          std::hypot(local_path.local_path[i + 1].pose.pose.position.y -
                         local_path.local_path[i].pose.pose.position.y,
                     local_path.local_path[i + 1].pose.pose.position.x -
                         local_path.local_path[i].pose.pose.position.x);
      if (dis <= kMathEpsilon) {
        local_path.local_path[i].rkappa = 0.0;
      } else {
        local_path.local_path[i].rkappa = (local_path.local_path[i + 1].rkappa -
                                           local_path.local_path[i].rkappa) /
                                          dis;
      }
    } else {
      const double dis =
          std::hypot(local_path.local_path[i].pose.pose.position.y -
                         local_path.local_path[i - 1].pose.pose.position.y,
                     local_path.local_path[i].pose.pose.position.x -
                         local_path.local_path[i - 1].pose.pose.position.x);
      if (dis <= kMathEpsilon) {
        ...
        local_path.local_path[i].rkappa =
            (local_path.local_path[i].rkappa -
             local_path.local_path[i - 1].rkappa) /
            dis;
      }
    }
  }
