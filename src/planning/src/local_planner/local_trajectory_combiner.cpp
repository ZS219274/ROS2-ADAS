...
{
  RCLCPP_INFO(rclcpp::get_logger("local_trajectory_combiner.cpp"),
              "本地轨迹合成器已创建");
...
LocalTrajectory LocalTrajectoryCombiner::combine_trajectory(
    const LocalPath& path, const LocalSpeeds& speeds)  // 合成轨迹
{
  ...
  if (path_size < 3) {
    RCLCPP_WARN(rclcpp::get_logger("local_trajectory_combiner.cpp"),
                "路径点数或速度点数小于3");
  ...
  RCLCPP_INFO(rclcpp::get_logger("local_trajectory_combiner.cpp"),
              "轨迹合成完成,轨迹点数: %ld",
              local_trajectory_.local_trajectory.size());
  return local_trajectory_;
}
}  // namespace Planning