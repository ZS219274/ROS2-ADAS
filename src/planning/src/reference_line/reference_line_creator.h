...
class ReferenceLineCreator  // 参考线创建器
{
 public:
  ...
  Referline create_reference_line(
 ...
 public:
  ...
  inline Path reference_line_rviz() const {
    return refer_line_rviz_;
  }  // 获取参考线rviz显示
  inline int match_point_index() const {
 ...
 private:
  ...
  std::shared_ptr<ReferenceLineSmoother>
      reference_line_smoother_;      // 参考线平滑
