    ...
    const PoseStamped& target_point)  // 生成参考线
{
  ...
  // 寻找匹配点
  match_point_index_ =
      Curve::find_match_point(global_path, match_point_index_, target_point);
  last_match_point_index_ =
      match_point_index_;  // 保存刚找到的匹配点到上一帧（迭代）
  if (match_point_index_ < 0) {
    RCLCPP_ERROR(rclcpp::get_logger("reference_line_creator.cpp"),
                 "match point index error.");
    return refer_line_;
  }
  ...
  front_index_ =
      (global_path_size - 1 >=
       match_point_index_ + reference_line_config_->refer_line().front_size_)
          ? (match_point_index_ +
             reference_line_config_->refer_line().front_size_)
          : (global_path_size - 1);
  back_index_ = (0 <= match_point_index_ -
                          reference_line_config_->refer_line().back_size_)
                    ? (match_point_index_ -
                       reference_line_config_->refer_line().back_size_)
                    : 0;
  ...
  RCLCPP_INFO(rclcpp::get_logger("reference_line_creator.cpp"),
              "reference line created, match point index = %d, front index = "
              "%d, back index = %d. size = %ld",
              match_point_index_, front_index_, back_index_,
