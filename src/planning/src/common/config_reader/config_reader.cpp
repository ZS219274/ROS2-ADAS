...
void ConfigReader::read_vehicle_config(VehicleStruct& vehicle,
  ...
  vehicle.pose_theta_ =
      planning_config["vehicle"][name]["pose_theta"].as<double>();
  vehicle.speed_ori_ =
      planning_config["vehicle"][name]["speed_ori"].as<double>();
...
void ConfigReader::read_pnc_map_config() {
  try {
    ...
    pnc_map_.road_length_ =
        planning_config["pnc_map"]["road_length"].as<double>();
    pnc_map_.road_half_width_ =
        planning_config["pnc_map"]["road_half_width"].as<double>();
    pnc_map_.segment_len_ =
        planning_config["pnc_map"]["segment_len"].as<double>();
    pnc_map_.speed_limit_ =
        planning_config["pnc_map"]["speed_limit"].as<double>();
  } catch (const YAML::Exception& e) {
    RCLCPP_ERROR(rclcpp::get_logger("config"),
...
void ConfigReader::read_global_path_config() {
  try {
    ...
    RCLCPP_ERROR(rclcpp::get_logger("config"),
                 "Failed to load global_path config: %s", e.what());
  }
}
...
void ConfigReader::read_reference_line_config() {
  try {
    ...
    refer_line_.front_size_ =
        planning_config["reference_line"]["front_size"].as<int>();
    refer_line_.back_size_ =
        planning_config["reference_line"]["back_size"].as<int>();
  } catch (const YAML::Exception& e) {
    RCLCPP_ERROR(rclcpp::get_logger("config"),
                 "Failed to load reference_line config: %s", e.what());
  }
}
...
void ConfigReader::read_local_path_config() {
  try {
    ...
    local_path_.curve_type_ =
        planning_config["local_path"]["curve_type"].as<int>();
    local_path_.path_size_ =
        planning_config["local_path"]["path_size"].as<int>();
  } catch (const YAML::Exception& e) {
    RCLCPP_ERROR(rclcpp::get_logger("config"),
                 "Failed to load local_path config: %s", e.what());
  }
}
...
void ConfigReader::read_local_speeds_config() {
  try {
    read_pnc_map_config();
    local_speeds_.speed_size_ =
        planning_config["local_speed"]["speed_size"].as<int>();
  } catch (const YAML::Exception& e) {
    RCLCPP_ERROR(rclcpp::get_logger("config"),
                 "Failed to load local_speeds config: %s", e.what());
  }
}
...
void ConfigReader::read_decision_config() {
  try {
    ...
    decision_.safe_dis_l_ =
        planning_config["decision"]["safe_dis_l"].as<double>();
    decision_.safe_dis_s_ =
        planning_config["decision"]["safe_dis_s"].as<double>();
  } catch (const YAML::Exception& e) {
    RCLCPP_ERROR(rclcpp::get_logger("config"),
                 "Failed to load decision config: %s", e.what());
  }
}
...
void ConfigReader::read_planning_process_config() {
  try {
    ...
    process_.obs_dis_ =
        planning_config["planning_process"]["obs_dis"].as<double>();
  } catch (const YAML::Exception& e) {
    RCLCPP_ERROR(rclcpp::get_logger("config"),
                 "Failed to load planning_process config: %s", e.what());
  }
}
...
void ConfigReader::read_move_cmd_config() {
  try {
    ...
    RCLCPP_ERROR(rclcpp::get_logger("config"),
                 "Failed to load move_cmd config: %s", e.what());
  }
}
}  // namespace Planning
