...
// 定位点转frenet
void MainCar::vehicle_cartesian_to_frenet(
    const Referline& refer_line)  // 定位点在参考线上的投影点参数
{
  ...
  Curve::find_projection_point(refer_line, local_point_, rs, rx, ry, rtheta,
                               rkappa, rdkappa);
  RCLCPP_INFO(rclcpp::get_logger("main_car_info.cpp"),
              "主车的投影点: rs = %.2f, rx = %.2f, ry = %.2f, rtheta = %.2f, "
  ...
  RCLCPP_INFO(
      rclcpp::get_logger("main_car_info.cpp"),
      "主车的frence坐标下的参数: s = %.2f, ds_dt = %.2f, dds_dt = %.2f, l = "
