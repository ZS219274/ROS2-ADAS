    ...
    : Node("planning_process")  // 规划流程
{
  ...
  global_path_client_ =
      this->create_client<GlobalPathService>("global_path_service");
  ...
  local_trajectory_pub_ =
      this->create_publisher<LocalTrajectory>("local_trajectory", 10);
}
...
bool PlanningProcess::process()  // 总流程
{
  ...
  timer_ = this->create_wall_timer(
      0.1s,
      std::bind(&PlanningProcess::planning_callback, this));  // 0.1s为周期
...
bool PlanningProcess::planning_init()  // 规划初始化
{
  ...
  if (!connect_to_server(global_path_client_)) {
    RCLCPP_ERROR(this->get_logger(),
                 "Failed to connect to global_path_server!");
    return false;
  }
...
void PlanningProcess::vehicle_spawn(
    const std::shared_ptr<VehicleInfoBase>& vehicle)  // 生成车辆
{
  TransformStamped spawn;
  spawn.header.stamp = this->now();
  spawn.header.frame_id =
      planning_process_config_->pnc_map().frame_;  // 地图坐标
  spawn.child_frame_id = vehicle->child_frame();   // 地图坐标的子坐标为车辆坐标

  spawn.transform.translation.x =
      vehicle->local_point().pose.position.x;  // 车辆位姿初始化
  ...
  spawn.transform.rotation.x =
      vehicle->local_point().pose.orientation.x;  // 四元数初始化
  ...
  RCLCPP_INFO(this->get_logger(), "vehicle %s spawned, x = %.2f, y = %.2f",
              spawn.child_frame_id.c_str(), spawn.transform.translation.x,
              spawn.transform.translation.y);
  tf_broadcaster_->sendTransform(spawn);  // 广播出去
}

void PlanningProcess::get_location(
    const std::shared_ptr<VehicleInfoBase>& vehicle)  // 获取车辆位置
{
  try {
    PoseStamped point;
    auto ts =
        buffer_->lookupTransform(planning_process_config_->pnc_map().frame_,
...
bool PlanningProcess::connect_to_server(const T& client)  // 连接服务
{
  ...
  // 判断客户端类型
  if constexpr (std::is_same_v<T, rclcpp::Client<PNCMapService>::
                                      SharedPtr>)  // c++17引入的新特性
  {
    server_name = "pnc_map_server";
  } else if constexpr (std::is_same_v<
                           T, rclcpp::Client<GlobalPathService>::SharedPtr>) {
  ...
  while (!client->wait_for_service(1s)) {
    ...
    RCLCPP_WARN(this->get_logger(), "Waiting for %s to be available...",
                server_name.c_str());
  }
  return true;
}
...
bool PlanningProcess::global_path_request()  // 全局路径请求
{
  RCLCPP_INFO(this->get_logger(),
              "Requesting global_path from global_path_server...");
...
void PlanningProcess::planning_callback()  // 总流程回调
{
  ...
  for (const auto& obs : obs_spawn_) {
    get_location(obs);
    if (std::hypot(obs->local_point().pose.position.x -
                       car_->local_point().pose.position.x,
                   obs->local_point().pose.position.y -
  ...
  // 参考线
  const auto refer_line = reference_line_creator_->create_reference_line(
      global_path_, car_->local_point());
  if (refer_line.refer_line.empty()) {
    RCLCPP_INFO(this->get_logger(), "Reference Line empty.");
    return;
  }

  const auto refer_line_rviz =
      reference_line_creator_->reference_line_to_rviz();  // 显示参考线
  ...
  std::sort(obses_.begin(), obses_.end(),
            [](const std::shared_ptr<VehicleInfoBase>& obs1,
               const std::shared_ptr<VehicleInfoBase>& obs2) {
  ...
  // 路径规划
  const auto local_path = local_path_planner_->create_local_path(
      refer_line, car_, decision_center_);  // 生成局部路径
  if (local_path.local_path.empty()) {
    RCLCPP_ERROR(this->get_logger(), "局部路径为空");
    return;
  }
  const auto local_path_rviz =
      local_path_planner_->path_to_rviz();    // 生成rviz的局部路径
  ...
  const auto local_trajectory =
      local_trajectory_combiner_->combine_trajectory(local_path, local_speeds);
  ...
  RCLCPP_INFO(this->get_logger(),
              "----car state: loc: (%.2f, %.2f), speed: %.2f, acceleration: "
              "%.2f, theta: %.2f, kappa: %.2f,",
              car_->local_point().pose.position.x,
              car_->local_point().pose.position.y, car_->speed(),
