...
{
  RCLCPP_INFO(rclcpp::get_logger("reference_line_smoother.cpp"),
              "Reference line smoother has been created");
...
void ReferenceLineSmoother::smooth_reference_line(
    Referline& refer_line)  // 平滑参考线
{
  ...
  Eigen::MatrixXd P_tmp =
      Eigen::MatrixXd::Zero(2 * n, 2 * n);  // 初始化为全0矩阵
  ...
  Eigen::VectorXd buff = Eigen::VectorXd::Constant(
      2 * n, 0.2);  // 偏差范围，动态列向量，2 * n行，值全为0.2
  buff(0) = buff(1) = buff(2 * n - 2) = buff(2 * n - 1) =
      0.0;  // 第0、1、2n-2、2n-1行偏差范围为0
  ...
  if (!solver.data()->setHessianMatrix(P)) {
    RCLCPP_ERROR(rclcpp::get_logger("reference_line_smoother.cpp"),
                 "设置二次型矩阵失败");
    return;
  }
