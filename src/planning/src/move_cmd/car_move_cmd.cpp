...
namespace Planning {
CarMoveCmd::CarMoveCmd()
    : Node("car_move_cmd_node")  // 主车运动指令
{
...
void CarMoveCmd::car_broadcast_tf(
    const LocalTrajectory::SharedPtr trajectory)  // 广播主车坐标变换
{
  // 接受轨迹
  ...
  transform_data.transform.translation.x =
      trajectory->local_trajectory[closest_index]
          .path_point.pose.pose.position.x;
  transform_data.transform.translation.y =
      trajectory->local_trajectory[closest_index]
          .path_point.pose.pose.position.y;
  ...
  transform_data.transform.rotation.x =
      trajectory->local_trajectory[closest_index]
          .path_point.pose.pose.orientation.x;
  transform_data.transform.rotation.y =
      trajectory->local_trajectory[closest_index]
          .path_point.pose.pose.orientation.y;
  transform_data.transform.rotation.z =
      trajectory->local_trajectory[closest_index]
          .path_point.pose.pose.orientation.z;
  transform_data.transform.rotation.w =
      trajectory->local_trajectory[closest_index]
          .path_point.pose.pose.orientation.w;

  // 广播消息
  RCLCPP_INFO(this->get_logger(),
              "广播主车坐标变换, pos:(%.2f, %.2f),speed:(%.2f, %.2f), "
              "local_trajectory_size: %ld",
              car_param_.pose_x_, car_param_.pose_y_, speed_x, speed_y,
              trajectory->local_trajectory.size());
