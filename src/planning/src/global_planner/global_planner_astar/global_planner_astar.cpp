...
Path AStar::search_global_path(const PNCMap& pnc_map)  // 全局路径搜索
{
  ...
  double start_x = (pnc_map.midline.points.front().x +
                    pnc_map.right_boundary.points.front().x) /
                   2.0;
  double start_y = (pnc_map.midline.points.front().y +
                    pnc_map.right_boundary.points.front().y) /
                   2.0;
  // 目标点如果是道路终点其实是不合理的，目标点应该是一个指定的坐标（后期修改）
  double goal_x = (pnc_map.midline.points.back().x +
                   pnc_map.right_boundary.points.back().x) /
                  2.0;
  double goal_y = (pnc_map.midline.points.back().y +
  ...
  for (int attempt = 0; attempt <= MAX_RETRY_ATTEMPTS; ++attempt) {
    ...
    std::unordered_set<int> allowed_points = buildAllowedPointsSet(
        pnc_map, origin_x, origin_y, RESOLUTION, grid_width, grid_height);

    if (start_grid_x < 0 || start_grid_x >= grid_width || start_grid_y < 0 ||
        start_grid_y >= grid_height) {
      RCLCPP_ERROR(rclcpp::get_logger("global_planner_astar.cpp"),
                   "Start point is out of map bounds");
    }

    if (goal_grid_x < 0 || goal_grid_x >= grid_width || goal_grid_y < 0 ||
        goal_grid_y >= grid_height) {
      RCLCPP_ERROR(rclcpp::get_logger("global_planner_astar.cpp"),
    ...
    int start_h =
        static_cast<int>(sqrt(static_cast<double>(dx * dx + dy * dy)) * 10);
    int start_key = start_grid_x * grid_width + start_grid_y;
    Node* start_node =
        new Node(start_grid_x, start_grid_y, 0, start_h, start_h, nullptr);
...
std::unordered_set<int> AStar::buildAllowedPointsSet(
    const PNCMap& pnc_map, double origin_x, double origin_y, double resolution,
